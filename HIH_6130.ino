...
Edditted and rewritten by Ben Holland for Digital Electronics, PHGN317 at the Colorado School of Mines
Nov. 2015
...
byte fetch_humidity_temperature(unsigned int *p_H_dat, unsigned int *p_T_dat) //Fetch Humidity value from HIH-6130, send it back via variables 1 and 2 (p_H_dat) and (p_T_dat) (Using pass by reference)
{
	...
	hih_addr = 0x27;  //HIH-6130 I2C adress
	Wire.beginTransmission(hih_addr);  //Start trasnmission HIH-6130
	...
	Serial.println((byte)Hum_H,BIN);
	Serial.println((byte)Hum_L,BIN);
...
/*
print_float(floaf f. int num_digits)
Print a float, f, of num_digits long. 
*/
